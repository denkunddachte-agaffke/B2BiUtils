 ...
 * @author chef
 *
 * Utility to wrap text strings. 
 * Based on org.apache.commons.text.WordUtils from Apache commons-text library 
 ...
 */
 public class WordUtil {
    ...
    try (Scanner sc = new Scanner(str)) {
      while (sc.hasNextLine()) {
        if (wrappedLine.length() > 0)
          wrappedLine.append(newLineStr.replace("$1", ""));
        wrapLine(wrappedLine, sc.nextLine(), wrapLength, newLineStr, wrapLongWords, wrapOn);
      }
    }
   ...
   * @return a line with newlines inserted, {@code null} if null input
   */
  private static void wrapLine(final StringBuilder wrappedLine, final String str, int wrapLength, String newLineStr, final boolean wrapLongWords, String wrapOn) {
    if (str == null) {
      return;
    }
    if (newLineStr == null) {
    ...
    if (StringUtils.isNullOrWhiteSpace(wrapOn)) {
      wrapOn = " ";
    }
    String nl = newLineStr;
    ...
    while (offset < inputLineLength) {
      ...
      if (matcher.find()) {
        ...
        if (matcher.groupCount() > 0)
          nl = newLineStr.replace("$1", matcher.group(1));
      }
    ...
    if (offset == 1 || ( matcherSize == 0 && offset < inputLineLength)) {
      offset--;
    }
